...
if __name__ == "__main__":
    api = Api()
    runs = api.runs("schroedinger_univie/gao_shared_C2")
    runs = [r for r in runs if r.name.startswith("reuseshared_18x20_v10_500k_C2_Booth2011")]
    print(f"Found {len(runs)} matching runs")

    metadata = dict(
        experiment="2023-05-26_C2_dimer_Booth2011_gao_reuseshared_from_tinymol_500k",
        source="dpe",
        method="reuseshared",
        embedding="dpe256",
        orbitals="gao_4fd",
        reuse_from="500kshared_tinymol_v10",
        n_pretrain_variational=500_000,
        n_shared_molecules=14,
        batch_size=2048,
    )
