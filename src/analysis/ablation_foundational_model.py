...
def get_model_size(orbitals_name):
    if orbitals_name.endswith("_S"):
        ...
        return 1
    elif orbitals_name.endswith("_XL"):
        return 3
    else:
        return 2
...
N_EPOCHS_FINETUNE = 1000

all_geometries = load_geometries()
all_datasets = load_datasets()
in_distr_geometries = [g.hash for g in all_datasets["TinyMol_CNO_rot_dist_test_in_distribution_30geoms"].get_geometries(all_geometries, all_datasets)]
out_of_distr_geometries = [g.hash for g in all_datasets["TinyMol_CNO_rot_dist_test_in_distribution_30geoms"].get_geometries(all_geometries, all_datasets)]

experiments = ['2023-03-06_tinymol_v10_ablation_n_pretrain', '2023-03-24_tinymol_v20_ablation']
...
plot_kwargs = dict(label_dict={True: "Small molecules", False: "Large molecules"},
...
ax_model.set_xticks([0, 1, 2, 3])
ax_model.set_xticklabels(["S", "M", "L", "XL"])
...
if save_figs:
    fig.savefig(f"/home/mscherbela/ucloud/results/ablation_foundational_model.png", dpi=400, bbox_inches="tight")
    fig.savefig(f"/home/mscherbela/ucloud/results/ablation_foundational_model.pdf", bbox_inches="tight")
...
for j, n in enumerate(n_epochs_fine):
    for i, n_pre in enumerate(n_epochs_pre):
        n_runs_in_distr[0][i,j] = len(df_full.query("n_pretrain_variational == @n_pre and epoch == @n and in_distribution == True and n_compounds == 18 and model_size == 2"))
        n_runs_out_of_distr[0][i,j] = len(df_full.query("n_pretrain_variational == @n_pre and epoch == @n and in_distribution == False and n_compounds == 18 and model_size == 2"))
    for i, n_comp in enumerate(n_compounds):
        n_runs_in_distr[1][i,j] = len(df_full.query("n_pretrain_variational == 256000 and epoch == @n and in_distribution == True and n_compounds == @n_comp and model_size == 2"))
        n_runs_out_of_distr[1][i,j] = len(df_full.query("n_pretrain_variational == 256000 and epoch == @n and in_distribution == False and n_compounds == @n_comp and model_size == 2"))
    for i, mod_size in enumerate(model_sizes):
        n_runs_in_distr[2][i,j] = len(df_full.query("n_pretrain_variational == 256000 and epoch == @n and in_distribution == True and n_compounds == 18 and model_size == @mod_size"))
        n_runs_out_of_distr[2][i,j] = len(df_full.query("n_pretrain_variational == 256000 and epoch == @n and in_distribution == False and n_compounds == 18 and model_size == @mod_size"))
