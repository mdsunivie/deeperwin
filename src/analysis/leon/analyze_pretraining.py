...
def load_wandb():
    ...
    filter_names = tuple(['prod_cas_NH3', 'prod_hf_NH3']) #
    ...
    for counter, run in enumerate(runs):
        ...
        print(method)
        try:
            d = dict(name=run.name, molecule=run.config['physical.name'], method=method, error_final=run.summary['error_eval'])
            intermed_error = [row for row in run.scan_history(keys=["opt_epoch", "error_intermed_eval", "sigma_intermed_eval"],
                                                              page_size=10_000)]
    ...
    print("finished")
    name = "pretrainingV3_FixedCAS.xlsx"
...
for i, m in enumerate(['NH3']): #df['molecule'].unique()
    ...
    for j, meth in enumerate(['prod_cas', 'prod_hf']):
        ...
        if meth == 'prod_cas':
            ...
            label = "Hartree Fock"
        ax[i].errorbar(data['nb_epochs'].unique(), data['error_final']['mean'], yerr=data['error_final']['std'],
                          label=f"{label} - 50k opt. epochs", color=color[2*j], capsize=3)
        ax[i].errorbar(data['nb_epochs'].unique(), data['error_intermed20000']['mean'],
                          yerr=data['error_intermed20000']['std'], label=f"{label} -  20k opt. epochs", color=color[2*j + 1], capsize=3)
    ...
    ax[i].grid(alpha=0.5)
    ax[i].set_title(f"$NH_3$")

ax[0].legend(loc='upper right', framealpha=0.9, fontsize=9)
ax[0].set_ylabel("Energy rel. to reference / mHa")
fig.tight_layout()
fig_fname = f"/Users/leongerard/Desktop/data_schroedinger/paper_plot/Pretraining_NH3.png"
