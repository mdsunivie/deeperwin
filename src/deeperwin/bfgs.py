...
def build_bfgs_optimizer(
    ...
    opt_init, opt_update, get_params = get_builtin_optimizer(bfgs_config.internal_optimizer, opt_config.schedule, opt_config.learning_rate)
    ...
    def optimize_epoch_bfgs(epoch, mcmc_state: MCMCState, opt_state, clipping_state, fixed_params):
        ...
        mcmc_state = mcmc.run_inter_steps(log_psi_squared_func, (unravel_pytree(get_params(inner_opt_state)), fixed_params), mcmc_state)
        ...
        def _batch_step(carry, r_batch):
            ...
            raw_grad, E_loc = grad_loss_flat(r_batch, mcmc_state.R, mcmc_state.Z, params_before_step, fixed_params, clipping_state)
        ...
        (params_new, inner_opt_state), (E_epoch, gradients) = jax.lax.scan(_batch_step, (jnp.zeros([n_params]), inner_opt_state), r_batches)
        ...
        def _update_hessian(inv_hessian):
            ...
            if bfgs_config.use_variance_reduction:
                ...
                y_new += grad_loss_flat(r_batches[0], mcmc_state.R, mcmc_state.Z, params_new, fixed_params, clipping_state)[0]
                y_new -= grad_loss_flat(r_batches[-1], mcmc_state.R, mcmc_state.Z, params_old, fixed_params, clipping_state)[0]
        ...
        inv_hessian = jax.lax.cond(epoch % bfgs_config.update_hessian_every_n_epochs == 0, _update_hessian, lambda x: x, inv_hessian)

        # Update clipping and wavefunction values
        clipping_state = calculate_clipping_state(E_epoch, opt_config.clipping)
        mcmc_state.log_psi_sqr = log_psi_squared_func(*mcmc_state.model_args, unravel_pytree(get_params(inner_opt_state)), fixed_params)
