...
def _parse_input_args(args=None):
    ...
    parser_setup = subparsers.add_parser(
        "setup", help="Setup and dispatch one or multiple DeepErwin calculations, e.g. for a parameter sweep"
    )
    ...
    parser_run_multiple_shared = subparsers.add_parser(
        "run-multiple-shared", help="Run a DeepErwin calculation for multiple molecules using weight-sharing"
    )
    ...
    parser_phisnet = subparsers.add_parser(
        "train-phisnet", help="Train a PhiSNet-like model to generate orbital embeddings"
    )
    ...
    subparsers.add_parser(
        "wandb-agent", help="Agent to be called by wandb sweep-agent for automatic hyperparameter search"
    )
    ...
    parser_restart.add_argument(
        "--force", "-f", action="store_true", help="Overwrite directories if they already exist"
    )
    ...
    parser_convert_chkpt = subparsers.add_parser(
        "convert-checkpoint",
    ...
    parser_merge_geom_db = subparsers.add_parser(
        "merge-geom-db",
    ...
    parser_merge_geom_db.add_argument(
        "--overwrite",
        action="store_true",
        help="Overwrites existing geometries/datasets with the same name. Default keeps the existing ones.",
    )

    from deeperwin.run_tools import queue_with_dependencies

    parser_queue = subparsers.add_parser(
        "queue-with-dependency", help="Run consecutive configs using slurm dependency system"
    )
