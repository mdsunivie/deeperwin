...
def build_evaluation_step(log_psi_sqr_func, mcmc, eval_config):
    def _evaluation_step(mcmc_state: MCMCState, params):
        ...
        if eval_config.forces is not None:
            poly_coeffs = _build_force_polynomial_coefficients(eval_config.forces.R_core, eval_config.forces.polynomial_degree)
            forces = calculate_forces(*mcmc_state.model_args, mcmc_state.log_psi_sqr, log_psi_sqr_func, params, eval_config.forces, poly_coeffs)
        else:
            forces = None
