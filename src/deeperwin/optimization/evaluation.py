...
from deeperwin.utils.utils import pmean, psum, replicate_across_devices, estimate_autocorrelation_time, get_distance_matrix
from deeperwin.utils.periodic import project_into_first_unit_cell
LOGGER = logging.getLogger("dpe")

@functools.partial(jax.pmap, axis_name="devices", static_broadcasted_argnums=(1,2,3,))
...
def evaluate_wavefunction(
    ...
    if get_local_energy_func is None:
        get_local_energy_func = build_local_energy(log_psi_sqr, 
                                            is_complex,
    ...
    def get_observables(params, fixed_params, spin_state: Tuple[int], mcmc_state: MCMCState):
        ...
        if config.calculate_energies:
            ...
            metrics['E_var'] = (pmean(jnp.nanmean((energies - metrics['E_mean']) * jnp.conj(energies - metrics['E_mean'])))).real
        if config.forces:
        ...
        if phys_config.periodic is not None and config.localization_metric is not None:
            z = pmean(jnp.mean(calculate_localization_z(mcmc_state.r, fixed_params['periodic'].rec),
                               axis=0))
            for dim in range(3):
        ...
        if config.density is not None:
            if "1el" in rho_cubes:
                for spin, el_slice in enumerate([slice(0, n_up), slice(n_up, None)]):
                    ...
                    rho_cubes["1el"][spin, ...] += get_density(mcmc_state.r[..., el_slice, :], 
                                                    grid_r_min, 
                                                    grid_r_max, 
                                                    grid_n,
                                                    lattice_replicated)[0]
            if "2el_rel" in rho_cubes:
                for spin1, el_slice1 in enumerate([slice(0, n_up), slice(n_up, None)]):
                    for spin2, el_slice2 in enumerate([slice(0, n_up), slice(n_up, None)]):
                        ...
                        rho_cubes["2el_rel"][spin1, spin2, ...] += get_density(diff, 
                                                                               grid_r_min, 
                                                                               grid_r_max, 
                                                                               grid_n,
                                                                               lattice_replicated)[0]
    ...
    if jax.process_index() == 0:
        ...
        if compute_structure_factors:
            ...
            if opt_epoch_nr is None:
                ...
                structure_fac_fname = f"{geom_id if geom_id is not None else ''}structure_factors_{opt_epoch_nr:06d}.npz"
    ...
    if "loc_z_0_real" in summary:
        for dim in range(3):
            ...
            summary[f"loc_abs_{dim}_sigma"] = np.sqrt(summary[f"loc_z_{dim}_real_sigma"]**2 + summary[f"loc_z_{dim}_imag_sigma"]**2)
            summary[f"loc_abs_{dim}_sigma_corr"] = np.sqrt(summary[f"loc_z_{dim}_real_sigma_corr"]**2 + summary[f"loc_z_{dim}_imag_sigma_corr"]**2)
