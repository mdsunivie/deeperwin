...
def _setup_environment(raw_config: Dict, config: Configuration) -> None:
    ...
    if config.reuse is not None:
        config, params_to_reuse, fixed_params, mcmc_state, opt_state, clipping_state, phisnet_params, map_fixed_params = load_data_for_reuse(config, raw_config)
    else:
        params_to_reuse, fixed_params, mcmc_state, opt_state, clipping_state, phisnet_params, map_fixed_params = None, None, None, None, None, None, None

    return root_logger, rng_seed, config, params_to_reuse, fixed_params, mcmc_state, opt_state, clipping_state, phisnet_params, map_fixed_params
...
def process_single_molecule(config_file: str):
    ...
    root_logger, rng_seed, config, params_to_reuse, fixed_params, mcmc_state, opt_state, clipping_state, phisnet_params_to_reuse, _ = _setup_environment(raw_config, config)
    ...
    tao_config = config.model.orbitals.transferable_atomic_orbitals

    if tao_config and tao_config.phisnet_model:
        N_ions_max = max([len(config.physical.Z)])
        all_elements = tao_config.atom_types or config.physical.Z
        all_atomic_orbitals = _get_all_basis_functions(all_elements, tao_config.basis_set)
        _, irreps = _get_orbital_mapping(all_atomic_orbitals, all_elements)
    ...
    log_psi_squared, orbital_func, cache_func, params, fixed_params = build_log_psi_squared(config.model, 
                                                                                            config.physical,
                                                                                            config.baseline, 
                                                                                            fixed_params, 
                                                                                            rng_seed, 
    ...
    if (config.evaluation.n_epochs > 0) and config.evaluation.evaluate_final and (config.optimization.n_epochs not in config.evaluation.opt_epochs):
...
def process_multiple_molecules_shared(config_file: str) -> None:
    ...
    raw_config, config = Configuration.load_configuration_file(config_file)
    config.save("full_config.yml")
    root_logger, rng_seed, config, params_to_reuse, fixed_params, mcmc_state, opt_state, clipping_state, phisnet_params_to_reuse, map_fixed_params = _setup_environment(raw_config, config)
    ...
    if tao_config and tao_config.phisnet_model:
        N_ions_max = max([len(pc.get_expanded_if_supercell().Z) for pc in physical_configs])
        all_elements = tao_config.atom_types or config.physical.Z
        all_atomic_orbitals = _get_all_basis_functions(all_elements, tao_config.basis_set)
        _, irreps = _get_orbital_mapping(all_atomic_orbitals, all_elements)
        phisnet_model = build_phisnet_model(phisnet_params_to_reuse,
    ...
    for idx, physical_config in enumerate(physical_configs):
        ...
        if g.fixed_params is None:
            g.fixed_params = init_model_fixed_params(config.model, physical_config, config.baseline, phisnet_model, N_ions_max)
    ...
    log_psi_squared, orbital_func, cache_func, params, fixed_params = build_log_psi_squared(config.model, 
                                                                                            [g.physical_config for g in geometries_data_stores], 
                                                                                            config.baseline,
                                                                                            fixed_params, 
                                                                                            rng_seed, 
                                                                                            phisnet_model, 
    ...
    has_final_been_evaluated = config.optimization.n_epochs * len(geometries_data_stores) in config.evaluation.opt_epochs
    ...
    # Finalize runs
    for g in geometries_data_stores:
        finalize_experiment_run(config,
