...
def run_orca(
    g: Geometry, directory, method, basis_set, frozen_core, n_proc, total_memory, orca_path="orca", clean_calc_dir=True
):
    write_orca_input(
        g,
        ...
        for basis_set in args.basis_set:
            for geom_hash in args.geometries:
                ...
                for g in geoms:
                    ...
                    t1 = time.time()
                    energy_file.write(
                        f"{ind_calc};{geom_hash};{method_string};{basis_set};{results.get('E_hf', '')};{results.get('E_final', '')};{t1-t0}\n"
                    )
                    ind_calc += 1
