...
optimizer = "kfac"
cache_fname = "/home/mscherbela/tmp/data_H4p_barrier.pkl"
...
colors = {
    ...
    "MRCI-D-F12": "k",
    "HF": "gray",
    "CCSD(T)-F12": "slategray",
    "Alijah2008": "C2",
}
linestyles = {"CCSD(T)-F12": "--", "Alijah2008": "None"}
molecule_name = dict(H4p="$H_4^+$", Ethene="Ethene")
curve_labels = {
    "Indep": "DeepErwin w/o weight-sharing",
    "Shared95": "DeepErwin w/ weight-sharing",
    "HF": "Hartree-Fock",
    "Alijah2008": "CAS AV7Z (Alijah 2008)",
}
...
for ind_mol, molecule in enumerate(["H4p", "Ethene"]):
    ...
    if molecule == "H4p":
        df_ref = pd.read_csv("/references/H4p_Barrier/H4plus_barrier.csv")
        df_ref["reaction_coord"] = df_ref["geom"] / (len(df_ref) - 1)
        indices_min = np.array([0, -1], int)
        index_barrier = 9
        ylims = [0, 8]
        images = [
            plt.imread(f"/home/mscherbela/ucloud/results/paper_figures/molecule_structures/povray/H4p_{i}crop.png")
            for i in [0, 2, 0]
        ]
        inset_ax_size = 0.30
        inset_ax_y = 0.75
    elif molecule == "Ethene":
        df_ref = pd.read_csv("/references/Ethene/Ethene_energies.csv")
        ...
        ylims = [0, 220]
        images = [
            plt.imread(f"/home/mscherbela/ucloud/results/paper_figures/molecule_structures/povray/Ethene.{i:02d}.png")
            for i in [0, 5, 9]
        ]
        inset_ax_size = 0.3
        inset_ax_y = 0.7
    ...
    for ref_method in ref_methods:
        ...
        ax.plot(
            df_ref.reaction_coord,
            1e3 * (df_ref[ref_method] - offset),
    ...
    for curve in full_plot_data[molecule]:
        ...
        if "Indep" in curve:
            df_out["DeepErwin without weight-sharing (blue)"] = full_plot_data[molecule]["Indep"][n_epochs]["E"]
        elif "Shared" in curve:
            df_out["DeepErwin with weight-sharing (red)"] = full_plot_data[molecule]["Shared95"][n_epochs]["E"]
    ...
    for i, img in enumerate(images):
        ...
        img_ax.imshow(img)
        img_ax.axis("off")
    df_out = df_out.rename(columns={"reaction_coord": "reaction coordinate"})
    df_out.to_csv(
        f"/home/mscherbela/ucloud/results/paper_figures/jax/figure_data/Fig4_barrier_{molecule}.csv", index=False
    )
    ax.text(0.0, 1.02, f"({chr(97 + ind_mol)})", dict(fontweight="bold", fontsize=14), transform=ax.transAxes)
