...
for ind_mol, molecule in enumerate(['H4p', 'Ethene']):
    ...
    for curve in full_plot_data[molecule]:
        ...
        if 'Indep' in curve:
            df_out[f'DeepErwin without weight-sharing (blue)'] = full_plot_data[molecule]['Indep'][n_epochs]['E']
        elif 'Shared' in curve:
            df_out[f'DeepErwin with weight-sharing (red)'] = full_plot_data[molecule]['Shared95'][n_epochs]['E']
    ...
    df_out.to_csv(f"/home/mscherbela/ucloud/results/paper_figures/jax/figure_data/Fig4_barrier_{molecule}.csv", index=False)
