...
#%%
    ...
    ax_scatter.scatter(df_with_ref.Fx_ref, df_with_ref.Fx, label=f"$F_x$: ($r^2$={r2_x:.3f})", color='C4', marker='D', alpha=0.7)
    ax_scatter.scatter(df_with_ref.Fa_ref, df_with_ref.Fa, label=f"$F_a$: ($r^2$={r2_a:.3f})", color='C3', marker='o', alpha=0.7)
    ...
    #
    df = df.rename(columns={'x': 'x in bohr', 'a': 'a in bohr', 'E': 'energy in Ha', 'E_ref': 'reference energy (MRCI) in Ha',
