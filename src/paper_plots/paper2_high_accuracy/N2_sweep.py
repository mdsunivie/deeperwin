...
data = data[
    (data.method != "fermi_block_32_200k")
    | data.bond_length.apply(lambda x: np.min(np.abs(x - bond_lengths_dpe)) < 0.05)
]
...
for n in [50, 100]:
    ...
    df_dpe = (
        ...
        .agg(
            ...
            energy_std=(key, "std"),
            n_runs=(key, "count"),
            energy_exp_interp=("energy_exp_interp", "mean"),
        )
        .reset_index()
    )
    ...
    df_dpe = (
        dpe_data[dpe_data.category.str.startswith("fermi")]
        .groupby(["bond_length"])
        .agg(
            energy=(key, "mean"),
            energy_std=(key, "std"),
...
ax_sweep_inset = inset_axes(
    ax_sweep,
    width="100%",
    height="100%",
    loc="upper left",
    bbox_to_anchor=(0.50, 1 - 0.29, 0.45, 0.25),
...
ax_sweep_inset.plot(
    data[data["method"] == "experiment"].bond_length, data[data["method"] == "experiment"].energy, color="dimgray"
)
...
for item in (
    [ax_sweep_inset.title, ax_sweep_inset.xaxis.label, ax_sweep_inset.yaxis.label]
    + ax_sweep_inset.get_xticklabels()
    + ax_sweep_inset.get_yticklabels()
):
    item.set_fontsize(9)
...
# fname = "/Users/leongerard/Desktop/data_schroedinger/N2_dissociation.png"
