...
if kind == "wide":
    fig, ax = plt.subplots(1, 3, dpi=100, figsize=(15, 4), gridspec_kw={"width_ratios": [1.0, 1.0, 1.2]})
else:
    fig, ax = plt.subplots(1, 3, dpi=100, figsize=(11, 4.5), gridspec_kw={"width_ratios": [1.0, 1.0, 1.2]})
...
plot_waterfall(
    ...
    horizontal=True,
    # pauli_waterfall=True,
    textcolors=["k", "k", "k", "w", "w", "w", "w", "w"],
    label_in_bar_thr=0.12,
    labeloffset=0.8,
)

ax[0].axvline(
    published_errors[molecule], color="k", ls="--", label="Best published\nvariational: [Pfau 2020],\n200k epochs"
)
# ax[0].text(0.05, 1.04, "a.)", ha='right', va='center', transform=ax[0].transAxes, weight='bold')
...
for i, key in enumerate(["ablation_pauli_01_NH3", "ablation_env_05_NH3", "copy_of_2_regression_nips_fulldet_NH3_tf32"]):
    ...
    if "ablation_env_05" in key:
        ...
        label = labels[2]
        color = color_dpe
    ax[1].plot(range(100, len(error)), error[100:], label=label, color=color)

ax[1].set_ylim([0, 18])
ax[1].set_xticks([0, 10000, 20000, 30000, 40000, 50000])
ax[1].set_xticklabels([0, 10, 20, 30, 40, 50])
ax[1].axhline(
    published_errors[molecule], color="k", ls="--", label="Best published\nvariational: [Pfau 2020],\n200k epochs"
)
ax[1].legend(loc="center right", framealpha=0.9, fontsize=9)
# ax[1].set_title(f"CASSCF vs. exp. envelope")
ax[1].set_xlabel("Epochs / 1k")
ax[1].grid(alpha=0.5)
ax[1].set_ylabel("Energy rel. to reference / mHa")
# ax[1].text(0.05, 1.04, "b.)", ha='right', va='center', transform=ax[1].transAxes, weight='bold')
...
color = [get_cmap("inferno")(0.0), get_cmap("inferno")(0.2), get_cmap("viridis")(0.7), get_cmap("viridis")(0.8)]
...
ax[2].set_xscale("log")
ax[2].set_xticks(list(data["nb_epochs"].unique()))
ax[2].set_xticklabels([0.2, 1, 5, 20, 100, 200])
...
ax[2].set_ylabel("Energy rel. to reference / mHa")
# ax[2].text(0.05, 1.04, "c.)", ha='right', va='center', transform=ax[2].transAxes, weight='bold')

fig.tight_layout()
fig_fname = f"/home/mscherbela/Nextcloud/PhD/talks_and_conferences/2022_IPAM/Talk/images/prior_physics_{kind}.png"
