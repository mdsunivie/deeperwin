...
titles = dict(Ethene='Ethene',
              N2_bond_breaking='$N_2$, $d=$4.0 bohr',
              K="K atom")

settings = ['01_fermi_iso', '02_fermi_iso_fulldet', '03_fermi_iso_fulldet_hp', '04_fermi_iso_fulldet_hp_emb', '05_dpe11', '06_dpe11_init']
labels_delta = ["Dense\nDeterminant", "Hyper-\nparameters", "SchNet-like\nembedding", "Local input-\nfeatures", "Envelope\nInitialization"]
E_ref = dict(Ethene=-78.588800, # CCSD(T)
             N2_bond_breaking=-109.202042, #experiment
             P=-341.259000, # experiment
             K=-599.9195443359196
             )
best_variational = dict(Ethene=(-78.5844, '[Pfau 2020]\n200k epochs'), #FermiNet block-det, 200k epochs'
                        # N2_bond_breaking=(-109.194, '[Lin 2021], 200k epochs'), #  FermiNet\n1 full-det, 200k epochs,
                        N2_bond_breaking=(-109.198817224834, '[Ren 2022]\n400k epochs'),
                        P=(-341.2578, '[Spencer 2021]')) # FermiNet 512, 300k
...
colors_delta = [color_other_improvements, color_our_improvements, color_our_improvements,
...
ablation_energies = ablation_energies_raw.groupby(['molecule', 'settings']).agg(
    error_mean=(metric, 'mean'), error_std=(metric, 'std'), error_min=(metric,'min'), error_max=(metric, 'max'),count=(metric, 'count')).reset_index()
...
for ind_mol, (ax, molecule) in enumerate(zip(axes, molecules)):
    ...
    if molecule in best_variational:
        ax.axvline(1e3 * (best_variational[molecule][0] - E_ref[molecule]), color='k', ls='--',
