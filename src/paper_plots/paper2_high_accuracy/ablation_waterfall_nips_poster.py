...
for plot_width in np.arange(4, 7.1, 0.5):
    # for plot_width in [5.0]:
    plt.close("all")
    fig, axes = plt.subplots(1, len(molecules), dpi=100, figsize=(plot_width, 4.4), sharey=True)
    for ind_mol, (ax, molecule) in enumerate(zip(axes, molecules)):
        ...
        ax.set_ylim([6.45, -0.45])

    fig.tight_layout()
    fig_fname = f"/home/mscherbela/ucloud/results/Talks/2022_NEURIPS/{'_'.join(molecules)}_{metric}_{plot_width}.png"
    fig.savefig(fig_fname, bbox_inches="tight", dpi=300)
