...
pivot = df.groupby(['is_local', 'geom']).agg(E_50k=('E_eval_50k', 'mean'),
                                             E_100k=('E_eval_100k', 'mean'),
                                             E_20k=('E_smooth_20k', 'mean')).reset_index()
...
for n_epochs in [20, 50, 100]:
    for is_local in [True, False]:
        ...
        axes[0].plot(df_filt.geom, df_filt[f'E_{n_epochs}k'], ls=ls,
                color=colors[n_epochs], label=f"{n_epochs}k: {'local feat.' if is_local else 'global feat.'}")
        axes[1].plot(df_filt.geom, (df_filt[f'E_{n_epochs}k']-df_filt.E_ref)*1e3, ls=ls,
                color=colors[n_epochs], label=f"{n_epochs}k: {'local feat.' if is_local else 'global feat.'}")
for ax in axes:
    ax.grid(alpha=0.5)
    ax.legend()
