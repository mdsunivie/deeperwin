...
error_gao, std_gao = data_tao.groupby(by="epoch").agg(np.mean).error.to_list(), data_tao.groupby(by="epoch").agg(np.std).error.to_list()
error_truncation, std_truncation = data_truncation.groupby(by="epoch").agg(np.mean).error.to_list(), data_tao.groupby(by="epoch").agg(np.std).error.to_list()
...
fig.tight_layout()
fig.savefig(f"/home/mscherbela/ucloud/results/03_paper_unversal_wavefuncion/figures/fig3_h2o_phaseflip.png", dpi=400, bbox_inches="tight")
fig.savefig(f"/home/mscherbela/ucloud/results/03_paper_unversal_wavefuncion/figures/fig3_h2o_phaseflip.pdf", bbox_inches="tight")
