...
df_spread = (
    df_full.groupby(["method", "molecule", "experiment", "epoch", "samples"])["error"]
    .agg(lambda x: np.max(x) - np.min(x))
    .reset_index()
)
df_relative = (
    df_full.groupby(["method", "molecule", "experiment", "epoch", "samples"])[["E", "E_ref"]]
    .agg(lambda x: np.max(x) - np.min(x))
    .reset_index()
)
...
hashes_in_order = [
    "00572fed1563d8dffead78f6206d159f",
    "68a9222ecdedfa1c28c47bff5f561f55",
    "b74f6a57c30a3e480b3da1f843210ed7",
    "84b0dea9a5b8580c5ad7556234c44aa4",
    "d90bf56c430e7b00af9f7159cc088373",
    "a08e544a9d4c501fdd0fd072cc74887e",
    "76bdef6a1423fb3014cb908ef6a0908f",
    "488f4bc14b1fdb0e935909eab7dfa107",
    "18097ae85beec490d7622cf897c84ee9",
    "fa8c21dfffb579a9f5d81cbddbaf2aba",
]
...
for molecule in ["HChain10", "C2H4", "Cyclobutadiene"]:
    ...
    ax_error.set_title(
        dict(
            C2H4="CH$_4$ $\\rightarrow$ C$_2$H$_4$",
            HChain10="H$_6$ $\\rightarrow$ H$_{10}$",
            Cyclobutadiene="C$_2$H$_4$ $\\rightarrow$ C$_4$H$_4$",
        )[molecule],
        fontsize=14,
    )
...
plot_df(ax_PES, df_reuse_final, x="twist", y="E", experiment="experiment", experiment_order=experiments, **plot_kwargs)
...
path_leon = "/Users/leongerard/ucloud/Shared"
img_ethene = Image.open(path_leon + "/results/03_paper_unversal_wavefuncion/figures/Ethene_twist.png")
img_ax.imshow(img_ethene)
img_ax.axis("off")

if save_figs:
    fig.savefig(
        path_leon + "/results/03_paper_unversal_wavefuncion/figures/reuse_from_smaller.png",
        dpi=400,
        bbox_inches="tight",
    )
    fig.savefig(
        path_leon + "/results/03_paper_unversal_wavefuncion/figures/reuse_from_smaller.pdf", bbox_inches="tight"
    )
