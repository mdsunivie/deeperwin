...
df_ref = df_full[
    df_full.experiment.str.startswith("HF_CBS") | df_full.experiment.str.startswith("CCSD(T)")
].pivot_table(index="geom", columns="experiment", values="E", aggfunc="mean")
...
plt.arrow(
    0.29, 0.75, 0, -0.39, length_includes_head=True, width=0.009, color="dimgray", transform=ax.transAxes, zorder=20
)
...
plt.arrow(
    0.85, 0.35, -0.59, 0, length_includes_head=True, width=0.007, color="dimgray", transform=ax.transAxes, zorder=20
)
plt.text(0.5, 0.35, "~20x faster", transform=ax.transAxes, ha="center", va="bottom")
plt.arrow(
    0.99, 0.24, 0, -0.1, length_includes_head=True, width=0.007, color="dimgray", transform=ax.transAxes, zorder=20
)
plt.text(0.98, 0.19, "~3x", transform=ax.transAxes, ha="right")

if save_figs:
    fig.savefig(
        "/home/mscherbela/ucloud/results/03_paper_unversal_wavefuncion/figures/eval_foundational_model_v3.png",
        dpi=400,
        bbox_inches="tight",
    )
    fig.savefig(
        "/home/mscherbela/ucloud/results/03_paper_unversal_wavefuncion/figures/eval_foundational_model_v3.pdf",
        bbox_inches="tight",
    )
