...
experiments = [
    ...
    ("2023-05-01_699torsion_nc_by_std_256k", "Our work (pre-trained)", "red", "-", "o"),
...
plt.close("all")
# 2x2 gridspec, 2 plots in first row, 1 plot in second row
fig, axes = plt.subplots(2, 1, figsize=(5, 7), height_ratios=[1, 1])
ax_finetune = axes[1]

ax = axes[0]
for ind_epoch, epochs in enumerate([0, 4000]):
    for exp_name, label, color, ls, marker in experiments:
        df_filt = df.query("experiment == @exp_name")
        df_filt = df_filt[
        ...
        if is_dpe:
            if epochs == 0:
                ...
                color = "red"
                label = f"{epochs // 1000}k fine-tuning steps"
            df_filt = df_filt.query("epoch == @epochs")
        if len(df_filt) == 0:
            continue
        print(exp_name, color)
        ax.errorbar(
            df_filt.n_heavy,
            df_filt["mean"],
    ...
    if ind_epoch == 0:
        ...
        ax.set_title("Accuracy across molecule size")
...
ax_finetune.set_title("Accuracy along fine-tuning (for 3 heavy atoms)")
ax_finetune.set_yscale("symlog", linthresh=10)
ax_finetune.set_ylim([-1, 10_000])
ax_finetune.set_ylabel("$E - E_\\mathrm{CCSD(T)-CBS}$ / mHa", labelpad=0)
...
fig.tight_layout()
fname = "/home/mscherbela/ucloud/results/04_paper_better_universal_wf/figures/fig3_absolute_energies_poster.png"
