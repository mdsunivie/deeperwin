...
datasets = {2: "Test_Set_2heavy_atoms_4geoms_dist",
...
df = df_all.groupby(["n_heavy", "experiment", "source", "method", "epoch"]).error.agg(["mean", "std", "count"]).reset_index()
...
for ind_epoch, (ax, epochs) in enumerate(zip(axes_n_heavy, [0, 4000])):
    for exp_name, label, color, ls, marker in experiments:
        df_filt = df.query(f"experiment == @exp_name and n_heavy >= 3")
...
for experiment, label, color, linestyle, marker in experiments:
    ...
    if is_ref:
        ...
        ax_finetune.text(np.sqrt(32000 * 16000), error * 1.05, label, color=color_cc, fontsize=10, ha="center", va="bottom")
    else:
        ax_finetune.errorbar(df_filt.epoch, 
                             df_filt["mean"], 
                             yerr=df_filt["std"],
                             label=label, 
                             color=color, 
