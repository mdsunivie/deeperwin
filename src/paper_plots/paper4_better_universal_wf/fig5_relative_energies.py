...
for (d, title, xlabel), ax in zip(datasets, axes):
    ...
    if d == "C3H4_dist_rotated_20geoms":
        ...
        ax.plot(pivot_ref[xaxis], (pivot_ref.E_ref - E0_ref)*1000, color="black", linestyle="-", label=ref_label, marker='^', zorder=1)
    if d == "Cylobutadiene_transition_6geoms":
        delta_E_ferminet = (154.676480107767 - 154.661385285574) * 1000
        ax.plot([0, 1], [0, delta_E_ferminet], color="gray", linestyle="none", label="FermiNet 200k steps", marker='s', markersize=6)

    img_fname = f"/home/mscherbela/ucloud/results/04_paper_better_universal_wf/figures/renders/relative_energies_mol{i+1}.png"
    if d == "N2_stretching_GerardEtAl2022":
...
ferminet_hartree_10k = np.array([40.4, 58.6, -25.2, -22.2, -27.9]) * KCAL_PER_MOL_IN_HARTREE + np.array([-0.28, -3.92, -2.95, -4.75, -1.93]) * KCAL_PER_MOL_IN_HARTREE
...
data_bicyclo = []
for structure, ferminet_10k, ferminet, dmc, ccsd_t in zip(structures, ferminet_hartree_10k, ferminet_hartree, dmc_hartree, ccsd_t_hartree):
...
#%%
pivot_for_latex = df_bicyclo.query("structure != 'bicbut' and method != 'dpe_1000'").pivot_table(index="structure", columns="method", values="error", aggfunc="mean")
