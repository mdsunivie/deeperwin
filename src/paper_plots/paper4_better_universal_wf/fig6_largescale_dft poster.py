...
# Scatter: E vs E_Dft
ax[0].plot([-315, -65], [-315, -65], color="black")
sns.scatterplot(
    data=df,
...
ax[0].set_ylabel("$E_\\mathrm{zero-shot}$ / Ha")
...
ax[1].set_xlim([-130, 320])
ax[1].set_xlabel("Energy residual: E - $E_\\mathrm{PBE0+MBD}$ / mHa")

handles, labels = ax[0].get_legend_handles_labels()
ax[1].legend_.remove()
ax[0].legend(handles, labels, loc="upper left", ncols=2, title="Nr. of heavy atoms")
fig.suptitle("Zero-shot accuracy on QM7-X")


# # Scatter: E diff vs Rmax
# sns.scatterplot(data=df[df['Nb. heavy atoms'] >= max_heavy_atoms], x=label_rmax, y="E - $E_\\mathrm{PBE0+MBD}$ / mHa", hue="Nb. heavy atoms", palette="deep", ax=ax[2], s=100, zorder=10, legend=False)
...
fig.tight_layout()
fname = "/home/mscherbela/ucloud/results/04_paper_better_universal_wf/figures/fig6_largescale_vs_dft_poster.png"
