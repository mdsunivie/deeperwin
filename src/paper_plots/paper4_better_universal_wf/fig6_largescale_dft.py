...
from deeperwin.run_tools.geometry_database import load_geometries, append_energies, get_all_geometries, load_energies
from deeperwin.utils.utils import get_distance_matrix
from deeperwin.orbitals import get_cosine_dist, align_orbitals
...
def _lin_reg(df_subset, x_pred, log=False):
    x, y = np.array(df_subset[label_rmax]).reshape(-1, 1), np.array(df_subset["E - $E_\\mathrm{PBE0+MBD}$ / mHa"]).reshape(-1, 1)
...
sns.scatterplot(data=df, x="$E_\\mathrm{PBE0+MBD}$ / Ha", y="E / Ha", style="Nb. heavy atoms", hue="Nb. heavy atoms", markers=marker_option, ax=ax[0], s=100, zorder=10, legend=True)
...
sns.histplot(data=df, x='E - $E_\\mathrm{PBE0+MBD}$ / mHa', hue="Nb. heavy atoms", ax=ax[1], kde=False, multiple="stack", legend=False)
...
sns.scatterplot(data=df[df['Nb. heavy atoms'] >= max_heavy_atoms], x=label_rmax, y="E - $E_\\mathrm{PBE0+MBD}$ / mHa", hue="Nb. heavy atoms", ax=ax[2], style="Nb. heavy atoms", markers=marker_option, s=100, zorder=10, legend=False)
...
fig.tight_layout()
fname = "fig6_largescale_vs_dft.png"
