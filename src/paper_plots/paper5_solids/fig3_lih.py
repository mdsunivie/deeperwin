...
df_ourwork = (
    df_ourwork.groupby(by="geom")
    .apply(lambda x: np.average(x.E_mean_corrected, weights=x.tabc_weight))
    .reset_index()
)
...
df_ourwork_lih333["E_mean_corrected"] = df_ourwork_lih333.E_mean + df_ourwork_lih333.sfc_correction + df_ourwork_lih333.zpve
...
df_deepsolid_lih333["E_mean_corrected"] = df_deepsolid_lih333.E_mean + df_deepsolid_lih333.hf_correction + df_deepsolid_lih333.zpve
...
ax.fill_between(geometries, -175.6, -174.9, color="darkgrey", label="Experiment") # experimental values go from -175.6mHa to -174.9mHa
