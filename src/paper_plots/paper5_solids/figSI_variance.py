...
for a in a_values:
    fname = os.path.join(basepath, f"plot_data/deepsolid_simulation_data/lih_a0_{a}.csv")
...
df_opt = df_all[df_all["type"] == "opt"].groupby("step")[["variance"]].sum().reset_index()

df_opt = df_all[df_all.type == "opt"].pivot_table(index="step", columns="a0", values="variance")
...
for fname, weight in zip(fnames, weights):
    df = pd.read_csv(os.path.join(basepath, fname))
    df["Var_weighted"] = df.variance * weight
    df["type"] = "opt"
    df.loc[df.index >= 300_000, "type"] = "eval"
    all_data.append(df)
df_all = pd.concat(all_data, ignore_index=True)
df_opt_graphene = df_all[df_all["type"] == "opt"].groupby("step")[["Var_weighted"]].sum().reset_index()
df_opt_graphene["Var_smooth"] = df_opt_graphene.Var_weighted.rolling(window=smoothing_window_graphene).mean()


# %%
# DeepErwin data for LiH
df_dpe = pd.read_csv(os.path.join(basepath, "plot_data/LiH_2x2x2_shared.csv"))
twists = [
    "0.000_0.000_0.000",
    "-0.400_-0.400_0.000",
    "-0.400_-0.400_-0.400",
...
df_dpe["Var_weighted_10tw"] = df_dpe.opt_E_var * df_dpe.twist.map(twist_weights)
df_dpe["Var_gamma"] = df_dpe.loc[df_dpe.twist == twists[0], "opt_E_var"]
df_dpe = df_dpe.pivot_table(index="opt_n_epoch", columns=["geom", "twist"], values=["Var_weighted_10tw", "Var_gamma"])
df_dpe = df_dpe.fillna(method="ffill")
df_dpe_smooth_LiH = pd.DataFrame(
    data=dict(
        step=df_dpe.index,
        Var_gamma=df_dpe["Var_gamma"].sum(axis=1).rolling(window=smoothing_window_LiH).mean() / 8 / 8,
        Var_10tw=df_dpe["Var_weighted_10tw"].sum(axis=1).rolling(window=smoothing_window_LiH).mean() / 8 / 8,
    )
)


# DeepErwin data
df_dpe = pd.read_csv(os.path.join(basepath, "plot_data/graphene_2x2_shared.csv"))
df_dpe_eval = pd.read_csv(os.path.join(basepath, "plot_data/graphene_data.csv"))
...
df_dpe["Var_weighted_3tw"] = df_dpe.opt_E_var * df_dpe.geom_hash.map(weights_3twists)
df_dpe["Var_weighted_19tw"] = df_dpe.opt_E_var * df_dpe.geom_hash.map(weights_19twists)
df_dpe = df_dpe.pivot_table(index="opt_n_epoch", columns="geom_hash", values=["Var_weighted_3tw", "Var_weighted_19tw"])
df_dpe = df_dpe.fillna(method="ffill")

df_dpe_smooth_graphene = pd.DataFrame(
    data=dict(
        step=df_dpe.index,
        Var_3tw=df_dpe["Var_weighted_3tw"].sum(axis=1).rolling(window=smoothing_window_graphene).mean() / 4,
        Var_19tw=df_dpe["Var_weighted_19tw"].sum(axis=1).rolling(window=smoothing_window_graphene).mean() / 4,
    )
)
...
# Plot graphene
ax_graphene.plot(
    3 * df_opt_graphene.step / 1000, df_opt_graphene.Var_smooth, label="DeepSolid (3$\\times$3 twist-grid)", color="C0"
)
ax_graphene.plot(
    df_dpe_smooth_graphene.step / 1000,
    df_dpe_smooth_graphene.Var_3tw,
    label="Ours (3$\\times$3 twist subset)",
    color="C1",
)
ax_graphene.plot(
    df_dpe_smooth_graphene.step / 1000,
    df_dpe_smooth_graphene.Var_19tw,
    label="Ours (12$\\times$12 twist-grid)",
    color="C3",
)
ax_graphene.set_xlabel("Total optimization steps / k")
ax_graphene.set_ylabel("Twist-averaged variance per unit cell / Ha")
# ax_graphene.set_ylim([-76.255, -76.200])
ax_graphene.legend()
ax_graphene.set_yscale("log")
...
ax_LiH.plot(df_dpe_smooth_LiH.step / 1000, df_dpe_smooth_LiH.Var_gamma, label="Ours ($\\Gamma$ subset)", color="C1")

ax_LiH.plot(
    df_dpe_smooth_LiH.step / 1000, df_dpe_smooth_LiH.Var_10tw, label="Ours (5$\\times$5$\\times$5 twist-grid)", color="C3"
)


ax_LiH.set_xlabel("Total optimization steps / k")
ax_LiH.set_ylabel("Averaged variance per unit cell / Ha")
# ax_LiH.set_ylim([-8.175, -8.11])
ax_LiH.legend()
ax_LiH.set_yscale("log")
...
fig.savefig(f"{save_dir}/figSI_variance_curves.pdf", bbox_inches="tight")
fig.savefig(f"{save_dir}/figSI_variance_curves.png", bbox_inches="tight", dpi=300)
plt.show()